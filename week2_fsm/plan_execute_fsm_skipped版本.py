...
TRANSITIONS = {
    ...
    ("EXECUTING", "skipped"): "EXECUTING",         # 本步无需工具，记录后推进
...
def reduce(state: AgentState, event: str, payload) -> AgentState:
    ...
    if event == "planned":
        ...
        # 推理型步骤：不产生工具结果，但要把"这一步交代了什么"记入结果流，
        # 否则 FINISHING 汇总时看不到 replan 写下的说明性步骤
        state.done_results.append(f"[本步跳过，无工具调用] {payload}")
        state.step_idx += 1
        state.retries = 0
...
def phase_execute(client, s: AgentState):
    ...
    system = (
        ...
        "仅当任务本身明确无需工具调用（如纯说明/舍弃/结论类步骤）时，"
        '才输出 {"tool": null, "reason": "为何无需调用"}。\n'
        f"可用工具：\n{render_tools()}\n\n"
    ...
    try:
        ...
        if name is None:  # 契约放宽：推理型步骤可声明"无需工具"，不再被逼出垃圾调用
            reason = action.get("reason", "未说明")
            print(
